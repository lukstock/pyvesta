...
reduction_parameters.extracted_dir = reduction_parameters.base_dir + '_extracted_red'
...
#initialize camera
camera = instruments.HARPS_camera_red()
...
if __name__ == '__main__':
    ...
    if os.path.exists(trace_filename):
        ...
        Trace_data = order_traces.trace_orders(master_orderdef_tracing, nsigmas=10)
        ...
        if datashare.instrument.use_linetilt:
            ...
            for fiber_nr in range(Trace_data.nr_of_fibers()):
                ...
                Trace_data.traces[fiber_nr] = ThAr_Peak_detection.fit_ThAr_lines(first_thar, Trace_data.traces[fiber_nr], peak_list, degx=datashare.instrument.tiltfit_pixeldeg, degy=datashare.instrument.tiltfit_orderdeg)
    ...
    if ExtractedHandler.exists(extracted_masterflat_name, Spectra.ObservationMode.FLAT):
        ...
        masterflat_spectrum = extraction.weighted_extract_optimal(flat_file , Trace_data, Weights, npools=datashare.reduction_parameters.npools_extract, filter_cosmics=True)
    ...
    if SolutionHandler.SolutionExists(ReferenceSolution_filename, modify_filename=False):
        ...
        if datashare.instrument.peaklinelist is None or datashare.instrument.peaklinelist == '':
            ...
            for thar_nr in range(RawHandler.number_of(Spectra.ObservationMode.THAR)):
                ...
                logging.info('\n\tTry to find wavelength solution for file {}'.format(basename))
