...
def _fit_tilt_singleorder(args):
    ...
    """
    #parameter whether to continue fitting
    cont_fitting = True

    #indices used to interpolate tilt over the order
    #do not use indices where tilt is nan
    use_inds = np.asarray(~np.isnan(order_rotations)).nonzero()

    #if no value left: No tilt
    if len(use_inds) < 1:
        median_tilt = 0.
        cont_fitting = False
    ...
    while cont_fitting:
        weights = window_sums[use_inds] / order_rotations_errs[use_inds]

        #median_tilt = float(np.average(order_rotations[use_inds])), weights=1./np.square(order_rotations_errs[use_inds])))
        #median_tilt = float(np.average(order_rotations[use_inds], weights=weights))


        #polynomial degree of fitting
        degree = np.min([deg, len(use_inds) -2]).astype(int)
        if deg > 0 and degree < 1:
            degree       = 0
            cont_fitting = False

        x = good_peaks[use_inds]

        if degree > 0:
            #fit chebyshev polynomials
            coeffs = np.polynomial.chebyshev.chebfit(x, order_rotations[use_inds], deg=degree, w=weights)

            #median tilt is an array, returns tilt at position x
            median_tilt = np.polynomial.chebyshev.chebval(x_range, coeffs)

            #determine outliners
            residuals = order_rotations[use_inds] - median_tilt[x.astype(int)]
            bad_inds = np.where(np.abs(residuals) > 5 * np.median(np.abs(residuals)))[0]

        #no fitting, just use mean
        else:
            median_tilt = np.average(order_rotations[use_inds], weights=weights)
            residuals = order_rotations[use_inds] - median_tilt
            bad_inds = np.where(np.abs(residuals) > 5 * np.median(np.abs(residuals)))[0]


        #stop fitting if no bad inds found or remaining indices would be less than 2
        if len(bad_inds) < 1 or len(use_inds) - len(bad_inds) < 2:
            cont_fitting = False
        else:
            use_inds = np.delete(use_inds, bad_inds)

    #calculate tilt error
    #only one or less tilts, error is 2 pi (full turn)
    if len(use_inds) < 2:
        median_tilt_err = 2 * np.pi
    else:
        #tilt is not constant
        if (not isinstance(median_tilt, numbers.Number)):
            median_tilt_err = float(np.sqrt(np.nansum(np.square(median_tilt[good_peaks[use_inds]] - order_rotations[use_inds]))) / len(use_inds))
        else:
            median_tilt_err  = float(np.sqrt(np.nansum(np.square(median_tilt - order_rotations[use_inds]))) / len(use_inds))

    #plot, if requested
    if plot:
        plt.errorbar(good_peaks, order_rotations, yerr=order_rotations_errs,fmt='ro')

        #tilt is not constant
        if (not isinstance(median_tilt, numbers.Number)):
            plt.plot(good_peaks ,np.ones_like(order_rotations) * median_tilt[good_peaks])
        else:
            plt.plot(good_peaks ,np.ones_like(order_rotations) * median_tilt)
        plt.show()

    #error is also 2 pi if calculation returns nan
    if np.isnan(median_tilt_err) or len(order_rotations) < 2:
        median_tilt_err = 2 * np.pi

    #no tilt if tilt is nan
    if isinstance(median_tilt, numbers.Number) and np.isnan(median_tilt):
        median_tilt = 0.
        median_tilt_err = 2 * np.pi

    print('median tilt: {} +- {}'.format(median_tilt, median_tilt_err))


    return (trace_nr, median_tilt, median_tilt_err)
    """
